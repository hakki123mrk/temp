...
class WhatsAppWebhookStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    // Lambda function using native handler (no external dependencies)
  ...
  try {
    ...
    // Handle order messages
      ...
      const itemsList = items.map(item => {
        ...
        return \`• \${item.quantity}x Product #\${item.product_retailer_id} - AED \${item.item_price.toFixed(2)} each\`;
      }).join('\\n');
      ...
      try {
        const middlewarePayload = {
          ...
          items: items.map(item => ({
            ...
            itemName: \`Product #\${item.product_retailer_id}\`,
        ...
        try {
          await sendInteractiveButtons(
            ...
            [
              {
                type: 'reply',
                reply: { id: \`confirm_addr_\${orderRef}\`, title: 'Use Saved Address' }
              },
              {
                type: 'reply',
                reply: { id: \`new_addr_\${orderRef}\`, title: 'Add New Address' }
              }
            ]
          );
        } catch (err) {
          console.error('Error sending address request:', err);
        }
      }, 2000);
    }
    ...
    // Handle interactive button replies
      ...
      if (buttonReply) {
        ...
        if (buttonId.startsWith('confirm_addr_')) {
          const orderRef = buttonId.replace('confirm_addr_', '');
          await sendWhatsAppMessage(
            phoneNumberId, token, waId,
            \`Perfect! ✅ We'll use your saved address for order \${orderRef}.\\n\\nProcessing your order now... 🚀\`
          );
        } else if (buttonId.startsWith('new_addr_')) {
          const orderRef = buttonId.replace('new_addr_', '');
          await sendWhatsAppMessage(
            phoneNumberId, token, waId,
            \`Please provide your delivery address for order \${orderRef}:\\n\\n📍 Format:\\nStreet, Building, Area, City\`
          );
        }
      }
    }
    
    // Handle address message (Flow response)
    else if (messageType === 'flow' || messageType === 'nfm_reply') {
      console.log('Flow/Address response received:', message);
      await sendWhatsAppMessage(
        phoneNumberId, token, waId,
